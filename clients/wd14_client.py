...
从 nai_draw_plugin core/retag/wd14_client.py 移植，关键差异：gradio_client 改为软依赖，
缺失时构造不抛、调用时返回 WD14ClientError，让 PNG 元数据反推可独立运行。
仅改 logger 名（get_logger("bizyair_generate_image_plugin")）。
"""
...
class WD14Client:
        ...
        *,
        ...
        if not GRADIO_AVAILABLE:
            self.logger.warning(
                ...
                "如需启用请执行: pip install gradio_client"
            )
    ...
    def _process_official_result(self, result: Any, threshold: float) -> Dict[str, Any]:
        ...
        if isinstance(output_characters, dict) and 'confidences' in output_characters:
            for item in output_characters['confidences']:
                ...
                if label and score > 0:
                    character_tags.append({"label": label, "score": score})
                    all_tags.append({"label": label, "score": score})
        ...
        if isinstance(output_tags, dict) and 'confidences' in output_tags:
            for item in output_tags['confidences']:
                ...
                if label and score >= threshold:
                    general_tags.append({"label": label, "score": score})
                    all_tags.append({"label": label, "score": score})
        ...
        if isinstance(rating, dict) and 'confidences' in rating:
            for item in rating['confidences']:
                ...
                score = item.get('confidence', 0.0)
                if label and score > 0:
                    rating_tags.append({"label": f"rating:{label}", "score": score})
    ...
    def _process_pixai_result(self, result: Any, threshold: float) -> Dict[str, Any]:
        ...
        character_tags: List[Dict[str, Any]] = []
        general_tags: List[Dict[str, Any]] = []
        copyright_tags: List[Dict[str, Any]] = []
        ...
        if isinstance(raw_json, dict):
            ...
            if feature_scores:
                for label, score in feature_scores.items():
                    if score >= threshold:
                        general_tags.append({"label": label, "score": float(score)})
                        all_tags.append({"label": label, "score": float(score)})

            character_scores = raw_json.get('character_scores', {})
            if character_scores:
                for label, score in character_scores.items():
                    if score > 0:
                        character_tags.append({"label": label, "score": float(score)})
                        all_tags.append({"label": label, "score": float(score)})
            ...
            if ip_scores:
                for label, score in ip_scores.items():
                    if score > 0:
                        copyright_tags.append({"label": label, "score": float(score)})
    ...
    def _process_pixai_onnx_result(self, result: Any, threshold: float) -> Dict[str, Any]:
        """处理 PixAI-Tagger-ONNX Space 的返回结果

        `/run_inference` 返回 6 个元素：result[3]=角色概率、result[4]=通用概率。
        """
        ...
        for item in _extract_confidences(character_prob):
            ...
            if not isinstance(label, str) or not label or score is None:
                continue
            try:
                score_f = float(score)
            except Exception:
    ...
    def _process_danbooru_v4_result(self, result: Any, threshold: float) -> Dict[str, Any]:
        """处理 Danbooru V4 Space 的返回结果"""
        # 返回 5 个值：output_string, general_dict, artist_dict, character_dict, rating_dict
        ...
        if isinstance(artist_dict, dict) and 'confidences' in artist_dict:
            for item in artist_dict['confidences']:
                ...
                score = item.get('confidence', 0.0)
                if label and score > 0:
                    artist_tags.append({"label": label, "score": score})
    ...
    def format_tags_as_string(self, tag_data: Dict[str, Any], format: str = "danbooru") -> str:
        """将标签格式化为字符串（danbooru: tag1, tag2 / natural: 带置信度）"""
        tags = tag_data.get("tags", [])
        if format == "danbooru":
            return ", ".join([tag["label"] for tag in tags])
        lines = []
        for tag in tags[:20]:
            lines.append(f"{tag['label']} ({tag['score']:.2%})")
        return "\n".join(lines)


__all__ = ["WD14Client", "WD14ClientError"]
