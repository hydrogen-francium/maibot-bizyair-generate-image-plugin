...
def sanitize_prompt_for_content_filter(prompt: str) -> str:
    """对 prompt 做一次保守清洗：删高危词、替换边缘词，保留主体。"""
    if not prompt:
        return prompt
    text = prompt
...
def sanitize_input_values(input_values: dict, prompt_field_keys: Iterable[str] | None = None) -> dict:
    ...
    for key in list(sanitized.keys()):
        if key in keys_to_clean or str(key).lower().endswith(".prompt") or key == "prompt":
            ...
            if isinstance(value, str) and value.strip():
                sanitized[key] = sanitize_prompt_for_content_filter(value)
    return sanitized
