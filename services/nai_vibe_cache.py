# -*- coding: utf-8 -*-
"""Vibe Transfer cache_id 本地缓存（NewAPI §20.3.1 / §20.3.2）。
...
逐字移植 nai_draw ``core/services/vibe_cache.py``（纯 stdlib，框架无关）：仅改 normalize
来源、落库路径层级与文件名。故意不复用宿主 ``MaiBot.db``：缓存属于插件私有运行时状态，
跨重启复用即可，没必要污染宿主消息库（与 services/llm_value_cache.py 的 data/ 落盘一致）。
"""
...
from .nai_image_meta import normalize_image_base64
...
_DEFAULT_DB_FILENAME: str = "nai_vibe_cache.db"
_INSTANCE: Optional[VibeCacheService] = None
_INSTANCE_LOCK = threading.Lock()


def _default_db_path() -> Path:
    """落到本插件目录下的 ``data/nai_vibe_cache.db``，避免污染宿主数据库。

    本文件在 ``插件root/services/`` 下，``parents[1]`` 即插件根（nai_draw 原版在
    ``core/services/`` 两层下用 parents[2]，移植到 bizyair 单层 services/ 故改 parents[1]）。
    """
    return Path(__file__).resolve().parents[1] / "data" / _DEFAULT_DB_FILENAME
