# -*- coding: utf-8 -*-
"""NAI 提示词后处理工具（从 nai_draw_plugin core/utils/prompt_postprocessor.py 移植）。

本模块是**框架无关的纯逻辑**，不依赖任何 MaiBot SDK，可独立单测。

P0 接线范围：只有 :func:`strip_cjk_and_fullwidth` 会被 ``NaiChatInputValueBuilder``
在送 NewAPI 前调用——因为 NewAPI 文档 §8 规定 ``prompt`` / ``negative_prompt`` /
``characters[i].prompt`` 必须是英文，含 CJK 字符或全角符号一律 400（硬失败），
而 LLM 偶尔会漏译，纯配置兜不住，所以这一步必须在代码里做。

其余函数（:func:`sanitize_sfw_prompt` / :func:`normalize_prompt_order` /
:func:`remove_selfie_appearance_tags` 及其 ``characters[]`` 版本）一并移植以便后续
期次（P1 命令面 NSFW 开关、P2 多人通道）复用，但 **P0 不接线**：
- ``sanitize_sfw_prompt`` 会剔除 bikini / swimsuit / lingerie / cleavage 等轻量暴露 tag，
  而 bizyair 既有管线（english_free / english_outfit 模板）是**故意允许轻量暴露**的，
  P0 默认接线会与既有设计意图冲突，因此留给 P1 的 NSFW 开关按需启用。
- ``normalize_prompt_order`` 仅做"温和纠正"，NAI 对乱序 tag 鲁棒，P0 不必接。

设计说明（沿用 nai_draw 原注释）：
- 自拍模式：在用户未明确指定外貌时，去掉容易产生"随机外貌"的标签（如 black hair/long hair）
- 轻量排序：把人物数量/视角类标签前置，把 year xxxx 放到末尾
- 规则是"温和纠正"，不尝试完整实现 Danbooru 全分类排序（容易误判、反而更乱）。
"""
...
def sanitize_sfw_prompt(prompt: str) -> str:
    ...
    注意：bizyair 既有管线默认允许轻量暴露，本函数 P0 **不接线**，仅供 P1 的 NSFW
    开关在 SFW 模式下按需调用。
    """
    if not prompt or not prompt.strip():
        return prompt
...
# ==================== 结构化多角色后处理 ====================
# 这些 wrapper 复用上面的字符串实现，专门服务于 NewAPI `characters[]` 通道（P2）。
