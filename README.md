...
| `values` | `string` | 候选值列表，支持 JSON 数组字符串或每行一个值。可引用 `{参数名}` 和内置变量（详见下方**内置变量说明**） |
| `probability` | `float` | 触发概率（`0~1`），`1` 代表必定执行 |

### 内置变量说明

内置变量是插件在运行时自动构造的特殊占位符，可在以下两处引用：
- `custom_variables_config.custom_variables[].values`：`literal` 和 `llm` 两种模式均支持
- `bizyair_client.openapi_parameter_mappings[].value`：参数映射的值模板中同样支持
...
> **注意**：消息数越多，发送给 LLM 的 token 越多，请根据实际需要选择合适的数量。
